...
for ep_idx, episode_folder in enumerate(episode_folders):
    ...
    if rgb.shape[0] != eef.shape[0]:
        print(f"Episode {episode_folder} frame mismatch: rgb={rgb.shape[0]}, eef={eef.shape[0]}")
    ...
    if n_invalid > 0:
        print(f"{episode_folder}: Dropped {n_invalid}/{len(valid_mask)} invalid frames")
...
if len(invalid_log) > 0:
    ...
    print(f"Saved invalid frame log to: {log_path}")
